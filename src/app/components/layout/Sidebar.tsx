...
export function Sidebar() {
          ...
          <div>
            <div className="flex items-center group/nav">
              <NavLink
                ...
                className={({ isActive }) =>
                  cn(
                    "flex-1 flex items-center gap-2.5 px-2 py-1 rounded-md text-[13px] transition-all duration-100",
            ...
            {projectsExpanded && (
              <div className="ml-7 mt-0.5 space-y-0.5">
                <NavLink
                  to="/projects?filter=external"
                  onClick={closeSidebar}
                  className={() =>
                    cn(
                      "flex items-center gap-2 px-2 py-1 rounded-md text-[12px] transition-all",
                      location.pathname === "/projects" && location.search.includes("filter=external")
                        ? "bg-gray-200/70 text-gray-900 font-semibold"
                ...
                <NavLink
                  to="/projects?filter=internal"
                  onClick={closeSidebar}
                  className={() =>
                    cn(
                      "flex items-center gap-2 px-2 py-1 rounded-md text-[12px] transition-all",
                      location.pathname === "/projects" && location.search.includes("filter=internal")
                        ? "bg-gray-200/70 text-gray-900 font-semibold"
                        : "text-gray-500 hover:bg-gray-200/50 hover:text-gray-700"
                    )
                  }
      ...
      <DraggableNavItem key={id} id={id} groupId={groupId} moveItem={moveItem}>
        <div className="relative">
          <NavLink
            ...
            className={({ isActive }) =>
              cn(
                "flex items-center gap-2.5 px-2 py-1 rounded-md text-[13px] transition-all duration-100 whitespace-nowrap",
                isActive
                  ? "bg-gray-200/70 text-gray-900 font-semibold"
                  : "text-gray-600 hover:bg-gray-200/50 hover:text-gray-900"
              )
  ...
  return (
    ...
    >
      ...
      <div className="px-3 pt-3 pb-1 overflow-x-hidden shrink-0">
        <div className="flex items-center gap-2">
          ...
          <div className="flex-1 min-w-0">
            {org && allOrgs.length > 1 ? (
              <button onClick={() => setOrgSwitcherOpen(!orgSwitcherOpen)} className="flex items-center gap-1 text-[13px] font-semibold text-gray-800 hover:text-gray-900 transition-colors whitespace-nowrap">
                <span className="truncate max-w-[140px]">{org.name}</span>
                <ChevronsUpDown size={12} className="text-gray-400 shrink-0" />
              </button>
            ) : (
              <span className="text-[13px] font-semibold text-gray-800 whitespace-nowrap truncate">{org ? org.name : "Poten Manager"}</span>
            )}
          </div>
          <button
            onClick={toggleCollapse}
          ...
          <div className="absolute left-3 right-3 top-[40px] bg-white border border-gray-200 rounded-lg shadow-lg z-[71] py-1 animate-in fade-in slide-in-from-top-1 duration-150">
            {allOrgs.map((o) => (
              <button
                ...
                className={cn("w-full flex items-center gap-2 px-3 py-1.5 text-[13px] transition-colors", o.orgId === activeOrgId ? "bg-blue-50 text-blue-600" : "text-gray-600 hover:bg-gray-50")}
              >
      ...
      <div className="px-2 pt-2 pb-4 overflow-y-auto overflow-x-hidden flex-1 scrollbar-hide">
        <nav className="space-y-0.5">
          ...
          <NavLink
            ...
            className={({ isActive }) =>
              cn(
                "flex items-center gap-2.5 px-2 py-1 rounded-md text-[13px] transition-all duration-100",
                isActive
                  ? "bg-gray-200/70 text-gray-900 font-semibold"
                  : "text-gray-600 hover:bg-gray-200/50 hover:text-gray-900"
              )
            ...
            return (
              <div key={group.id} className="pt-4">
                <div className="px-2 pb-1">
                  <span className="text-[11px] font-medium text-gray-400">{ko ? group.labelKo : group.labelEn}</span>
                </div>
      ...
      <div className="px-2 pb-3 pt-0 overflow-hidden shrink-0">
        <div className="pt-2 border-t border-[#E8E8E4] space-y-0.5">
          ...
          >
            ...
            <span className="text-[13px] font-medium text-gray-700 truncate flex-1">{currentUser.name}</span>
          ...
          {members.filter(m => m.id !== currentUser.id).length > 0 && (
            <div>
              <button
                ...
                className="w-full flex items-center gap-1.5 px-2 py-1 rounded-md text-[11px] font-medium text-gray-400 hover:text-gray-600 hover:bg-gray-200/40 transition-colors"
              >
                ...
                <div className="space-y-0.5 pl-1">
                  {members.filter(m => m.id !== currentUser.id).map((member) => (
                    <button
                      ...
                      className="w-full flex items-center gap-2 px-2 py-1 rounded-md text-[12px] transition-all group/member text-gray-500 hover:bg-gray-200/40 hover:text-gray-700"
                    >
